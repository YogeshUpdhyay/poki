...
import kmatsLogo from '../../assets/kmatslogo.png'
...
function Credits() {
  ...
  const [kmatsTooltipOpen, setKmatsTooltipOpen] = useState(false);

  // Tooltip for Credits Pill
  const { refs: pillRefs, floatingStyles: pillStyles, context: pillContext } = useFloating({
    open: tooltipOpen,
    onOpenChange: setTooltipOpen,
    middleware: [offset({ mainAxis: -40, crossAxis: 70 })],
    whileElementsMounted: autoUpdate,
  });
  const pillClientPoint = useClientPoint(pillContext);
  const { getReferenceProps: getPillRefProps, getFloatingProps: getPillFloatProps } = useInteractions([pillClientPoint]);

  // Tooltip for KMats Logo
  const { refs: kmatsRefs, floatingStyles: kmatsStyles, context: kmatsContext } = useFloating({
    open: kmatsTooltipOpen,
    onOpenChange: setKmatsTooltipOpen,
    middleware: [offset({ mainAxis: -40, crossAxis: 0 })],
    whileElementsMounted: autoUpdate,
  });
  const kmatsClientPoint = useClientPoint(kmatsContext);
  const { getReferenceProps: getKmatsRefProps, getFloatingProps: getKmatsFloatProps } = useInteractions([kmatsClientPoint]);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {tooltipOpen && !hovered && (
          <div
            ref={pillRefs.setFloating}
            style={{ ...pillStyles, zIndex: 10000 }}
            {...getPillFloatProps()}
          >
            <motion.div
      ...
      <AnimatePresence>
        {kmatsTooltipOpen && (
          <div
            ref={kmatsRefs.setFloating}
            style={{ ...kmatsStyles, zIndex: 10000 }}
            {...getKmatsFloatProps()}
          >
            <motion.div
              initial="hidden" animate="visible" exit="hidden"
